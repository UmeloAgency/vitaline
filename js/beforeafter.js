    ...
    function beforeAfter(that) {
      if (jQuery(that).children().length == 2) {
        ...
        jQuery(that).attr(
          ...
          'position:' + baPosition + ';overflow:' + baOverflow,
        );
        ...
        separator_bullet.setAttribute(
          'style',
          'position:absolute;width:44px;height:44px;line-height:30px;padding:6px;text-align:center;top:50%;left:50%;transform:translate(-50%,-50%);border-radius:50%;background-color:#fff;',
        );

        var separator_content = document.createElement('DIV');
        separator_content.setAttribute(
          'style',
        ...
        var resize = function () {
          if (jQuery(div1).children().width() > 0)
            jQuery(div2).children().width(jQuery(div1).children().width());
          if (jQuery(div1).children().height() > 0)
        ...
        if (settings.movable) {
          ...
          if (settings.clickMove) {
            jQuery(div1).on('click', function () {
              ...
              var correctRight =
                oPercentage > 100 ? 100 : oPercentage < 0 ? 0 : oPercentage;
              correctRight = correctRight + '%';
              jQuery(div2).width(correctRight);
              jQuery(separator).css('right', correctRight);
              if (
                typeof settings.onMoving != 'undefined' &&
                oRight != jQuery(separator).css('right')
              ) {
                settings.onMoving();
              }
